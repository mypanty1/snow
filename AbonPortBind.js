...
//fix inputmode numeric for iphone
...
Vue.component('AbonPortBindSearchAbon',{
  template:`<div name="AbonPortBindSearchAbon">
    <div class="display-flex flex-direction-column gap-8px">
      ...
      <datalist id="datalist_accounts">
        <template v-if="currentTaskItem">
          <optgroup label="Текущий наряд">
            <option v-bind="currentTaskItem"></option>
          </optgroup>
        </template>
        <optgroup :label="!currentTaskItem?'Наряды':'Другие наряды'">
          <option v-for="item of tasksItemsFiltered" v-bind="item"></option>
        </optgroup>
      </datalist>
      <input-el label="ЛС" placeholder="ЛС" v-model="account" datalistId="datalist_accounts" clearable @onKeyUpEnter="searchAbonAgreementInternetServices" class="padding-unset">
      ...
      <AbonPortBindSelectAbonInternetService v-else-if="agreement" v-bind="{abonData,agreement,vgroups}" v-model="vg" @onVgUnblock="searchAbonAgreementInternetServices('update')"/>
    </div>
  </div>`,
  props:{},
  data:()=>({
